...
// =======================
const ROLL_PCT = { SLOT: 0.006, CASINO: 0.006, POKER: 0.003 };
...
let isProcessing = false; // small global lock for processing (prevents double click across heavy operations)
  ...
  overlay.innerHTML = `
    <div class="special-modal" role="dialog" aria-modal="true">
      ...
      <div style="margin-top:10px;">
        <label><b>Slot — rate 0.8%</b></label>
        <div class="special-input-group" id="slot08_inputs">
          <input id="slot08_1" type="text" placeholder="ID 1" />
          <input id="slot08_2" type="text" placeholder="ID 2" />
          <input id="slot08_3" type="text" placeholder="ID 3" />
          <input id="slot08_4" type="text" placeholder="ID 4" />
          <input id="slot08_5" type="text" placeholder="ID 5" />
        </div>
        <div style="text-align:right; margin-top:8px;">
          <button id="confirmSlot08" class="btn primary">Tambah Slot 0.8%</button>
        </div>
      </div>

      <div style="margin-top:10px;">
        <label><b>Slot — rate 0.75%</b></label>
        <div class="special-input-group" id="slot075_inputs">
          <input id="slot075_1" type="text" placeholder="ID 1" />
          <input id="slot075_2" type="text" placeholder="ID 2" />
          <input id="slot075_3" type="text" placeholder="ID 3" />
          <input id="slot075_4" type="text" placeholder="ID 4" />
          <input id="slot075_5" type="text" placeholder="ID 5" />
        </div>
        <div style="text-align:right; margin-top:8px;">
          <button id="confirmSlot075" class="btn primary">Tambah Slot 0.75%</button>
        </div>
      </div>

      <div style="margin-top:10px;">
        <label><b>Slot — rate 1%</b></label>
        <div class="special-input-group" id="slot1_inputs">
          <input id="slot1_1" type="text" placeholder="ID 1" />
          <input id="slot1_2" type="text" placeholder="ID 2" />
          <input id="slot1_3" type="text" placeholder="ID 3" />
          <input id="slot1_4" type="text" placeholder="ID 4" />
          <input id="slot1_5" type="text" placeholder="ID 5" />
        </div>
        <div style="text-align:right; margin-top:8px;">
          <button id="confirmSlot1" class="btn primary">Tambah Slot 1%</button>
        </div>
      </div>
      ...
      <div style="margin-top:10px;">
        <label><b>Casino — rate 0.75%</b></label>
        <div class="special-input-group" id="casino075_inputs">
          <input id="casino075_1" type="text" placeholder="ID 1" />
          <input id="casino075_2" type="text" placeholder="ID 2" />
          <input id="casino075_3" type="text" placeholder="ID 3" />
          <input id="casino075_4" type="text" placeholder="ID 4" />
          <input id="casino075_5" type="text" placeholder="ID 5" />
        </div>
        <div style="text-align:right; margin-top:8px;">
          <button id="confirmCasino075" class="btn primary">Tambah Casino 0.75%</button>
        </div>
      </div>

      <div style="margin-top:10px;">
        <label><b>Casino — rate 1%</b></label>
        <div class="special-input-group" id="casino1_inputs">
          <input id="casino1_1" type="text" placeholder="ID 1" />
          <input id="casino1_2" type="text" placeholder="ID 2" />
          <input id="casino1_3" type="text" placeholder="ID 3" />
          <input id="casino1_4" type="text" placeholder="ID 4" />
          <input id="casino1_5" type="text" placeholder="ID 5" />
        </div>
        <div style="text-align:right; margin-top:8px;">
          <button id="confirmCasino1" class="btn primary">Tambah Casino 1%</button>
        </div>
      </div>
...
function addSpecialCasesFromInputs(prefix, type, rate) {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (el && String(el.value || "").trim() !== "") {
      ...
      if (cleaned) {
        console.log("Menambahkan Special Case untuk ID:", cleaned, "Tipe:", type, "Rate:", rate);  // Log data yang ditambahkan
...
document.getElementById("confirmSlot08")?.addEventListener("click", () => addSpecialCasesFromInputs("slot08", "SLOT", 0.008));
document.getElementById("confirmSlot075")?.addEventListener("click", () => addSpecialCasesFromInputs("slot075", "SLOT", 0.0075));
document.getElementById("confirmSlot1")?.addEventListener("click", () => addSpecialCasesFromInputs("slot1", "SLOT", 0.01));
document.getElementById("confirmCasino08")?.addEventListener("click", () => addSpecialCasesFromInputs("casino08", "CASINO", 0.008));
document.getElementById("confirmCasino075")?.addEventListener("click", () => addSpecialCasesFromInputs("casino075", "CASINO", 0.0075));
document.getElementById("confirmCasino1")?.addEventListener("click", () => addSpecialCasesFromInputs("casino1", "CASINO", 0.01));
...
function renderAll() {
  ...
  allData.forEach(r => {
    ...
    if (!grouped[key]) grouped[key] = { NET_TO: 0 };
    grouped[key].NET_TO += r.NET_TO;
  });

  const pivot = {};
  Object.entries(grouped).forEach(([key, val]) => {
    const [id, gt] = key.split("|");
    if (!pivot[id]) pivot[id] = { SLOT: { NET_TO: 0 }, CASINO: { NET_TO: 0 }, POKER: { NET_TO: 0 } };
    pivot[id][gt].NET_TO = val.NET_TO;
  });

  // aggregates
  let totalSlot = 0, totalCasino = 0, totalPoker = 0, totalBonus = 0;
  finishing = [];
  let rowIndex = 2;
  Object.entries(pivot).forEach(([id, v]) => {
    const netSlot = v.SLOT.NET_TO;
    const netCasino = v.CASINO.NET_TO;
    const netPoker = v.POKER.NET_TO;
    ...
    const totalNum = rollSlotNum + rollCasinoNum + rollPokerNum;
    ...
    finishing.push({
      ID: id,
      "TO SLOT": netSlot,
      "Roll 0.6% SLOT": { f: `B${rowIndex}*${slotRate}` },
      "TO CASINO": netCasino,
      "Roll 0.6% CASINO": { f: `D${rowIndex}*${casinoRate}` },
      "TO POKER": netPoker,
      "Roll 0.3% POKER": { f: `F${rowIndex}*${ROLL_PCT.POKER}` },
      "UserID": id,
      "TOTAL": { f: `C${rowIndex}+E${rowIndex}+G${rowIndex}` },
      slotRate,
      casinoRate,
  ...
  if (summaryEl) {
    ...
    <h3>📊 Ringkasan</h3>
    <ul class="bullets">
      <li>🎰 Total Net TO Slot: <b>${totalSlot.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</b></li>
      <li>🃏 Total Net TO Casino: <b>${totalCasino.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</b></li>
      <li>♠️ Total Net TO Poker: <b>${totalPoker.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</b></li>
      <li class="bonus">💎 <u>Total Bonus</u> (Rollingan): <b>Rp ${totalBonus.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</b></li>
    </ul>
  `;
  }
  ...
  // Render Tabel Data
  const cols = [
    "ID", "TO SLOT", "Roll 0.6% SLOT", "TO CASINO", "Roll 0.6% CASINO", "TO POKER", "Roll 0.3% POKER", "UserID", "TOTAL", "Ref No"
  ];
  ...
  finalData = finishing.map((r) => ({
    ID: r.ID,
    TO_SLOT: r["TO SLOT"],
    RollSlotFormula: r["Roll 0.6% SLOT"].f,
...
async function downloadExcel() {
  ...
  // Judul utama (merge)
  ws.mergeCells("A1:J2"); // Hanya sampai J, tanpa K dan L
  ...
  // =======================
  // 3️⃣ Header kolom (hapus K dan L)
  // =======================
  const headers = [
    "ID",
    "TO SLOT",
  ...
  sortedData.forEach((r) => {
    ...
    // Formula otomatis
    row.getCell(3).value = { formula: `=B${rowNum}*${r.slotRate || ROLL_PCT.SLOT}` };
    row.getCell(5).value = { formula: `=D${rowNum}*${r.casinoRate || ROLL_PCT.CASINO}` };
    row.getCell(7).value = { formula: `=F${rowNum}*${ROLL_PCT.POKER}` };
    row.getCell(9).value = { formula: `=C${rowNum}+E${rowNum}+G${rowNum}` }; // Kolom Total tidak lagi termasuk K dan L

    // Format angka
    [2, 4, 6].forEach(i => (row.getCell(i).numFmt = "#,##0"));
    [3, 5, 7, 9].forEach(i => (row.getCell(i).numFmt = "#,##0"));
  });
...
// =======================
 ...
 // =======================
ws.autoFilter = { from: "A3", to: "J3" }; // Filter untuk kolom sampai j saja
